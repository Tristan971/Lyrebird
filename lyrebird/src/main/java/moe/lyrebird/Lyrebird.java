...
import moe.tristan.easyfxml.container.FxMicronautApplication;
 ...
 * <p>
 * This makes it so the spring application context {@link #context} fetches the default {@link FxUiManager} whose
...
@EnableCaching
@Import({LyrebirdServerClientConfiguration.class, EasyFxmlAutoConfiguration.class})
public class Lyrebird extends FxMicronautApplication {
    ...
    @Override
    protected BeanContext getContainer() {
        final ApplicationContext ctx = Micronaut.run(new Class[]{getClass(), FxMicronautApplication.class});
        return ctx;
    }

    @Override
    public void start(Stage primaryStage) {
        context.getBean(LyrebirdUiManager.class).startGui(primaryStage);
    }
    ...
    @Override
    public void stop() {
        context.getBean(CleanupService.class).executeCleanupOperations();
        super.stop();
        System.exit(0);
    }
